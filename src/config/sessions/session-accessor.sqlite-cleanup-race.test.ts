...
describe("SQLite lifecycle cleanup races", () => {
  ...
  it("releases the store writer while disk-budget eviction archives a transcript", async () => {
    const sessionKey = "agent:main:disk-budget-race-history";
    const historicalSessionId = "disk-budget-race-historical";
    const writerKey = "agent:main:disk-budget-race-writer";
    await replaceSessionEntry(
      { sessionKey, storePath },
      { sessionId: "disk-budget-race-current", updatedAt: Date.now() },
    );
    await replaceSqliteTranscriptEvents({ sessionKey, sessionId: historicalSessionId, storePath }, [
      { type: "session", id: historicalSessionId, content: "historical disk-budget transcript" },
    ]);
    await replaceSessionEntry(
      { sessionKey: writerKey, storePath },
      { sessionId: "disk-budget-race-writer-session", updatedAt: Date.now() },
    );
    ...
    const eviction = enforceSqliteSessionHistoryDiskBudget({
      mode: "enforce",
      maintenance: { maxDiskBytes: usage.totalBytes - 1, highWaterBytes: 0 },
      storePath,
    });
    await materializationStarted;
    const writer = replaceSessionEntry(
      ...
      {
        label: "progressed",
        sessionId: "disk-budget-race-writer-session",
        updatedAt: Date.now(),
      },
    );
    const progressedDuringMaterialization = await Promise.race([
    ...
    await expect(eviction).resolves.toMatchObject({ removedEntries: 1 });
    await expect(writer).resolves.toMatchObject({ label: "progressed" });
    expect(progressedDuringMaterialization).toBe(true);
  });
  ...
  it("releases the store writer while lifecycle cleanup archives a transcript", async () => {
    ...
    const writerKey = "agent:main:writer-during-lifecycle-cleanup";
    await replaceSessionEntry(
      { sessionKey: deletedKey, storePath },
