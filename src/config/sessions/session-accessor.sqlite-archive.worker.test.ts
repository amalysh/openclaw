...
describe("SQLite transcript archive worker", () => {
  ...
  it("keeps archive plans small across many generations of one logical session", async () => {
    const sessionKey = "agent:main:cron:bounded-archive-planning";
    const sessionIds = Array.from({ length: 24 }, (_, index) => `bounded-generation-${index}`);
    const payload = "bounded archive payload ".repeat(4_096);

    for (const sessionId of sessionIds) {
      await replaceSqliteTranscriptEvents({ sessionKey, sessionId, storePath }, [
        createTranscriptEvent(sessionId, payload),
      ]);
    }

    const database = openLifecycleTestDatabase(storePath);
    const plans = sessionIds.map((sessionId) =>
      planArchiveWorker(database, path.dirname(storePath), sessionId),
    );
    const serializedPlans = JSON.stringify(plans);

    expect(plans).toHaveLength(sessionIds.length);
    expect(plans.every((plan) => plan.snapshot.lastSeq !== null)).toBe(true);
    expect(plans.every((plan) => !("content" in plan))).toBe(true);
    expect(serializedPlans).not.toContain(payload);
    expect(Buffer.byteLength(serializedPlans)).toBeLessThan(sessionIds.length * 1_024);
  });
