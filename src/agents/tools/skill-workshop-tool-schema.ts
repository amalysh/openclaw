      ...
      action: stringEnum(
              ...
              : [...SKILL_WORKSHOP_ACTIONS],
        {
          description: proposalRevision
            ...
            : proposalOnly
              ? `create = new skill;${updateProposals ? " prepare_patch = authorize one exact non-empty old_string and return bounded context for a skill that cannot fit a complete read (only one prepared span may be active per skill); patch = targeted find-and-replace after a complete read or prepare_patch (empty old_string appends only after a complete read); read = complete existing live skill when it fits the selected-model budget, otherwise metadata without a partial body; update = full-body rewrite after a complete read;" : ""} revise = existing pending proposal; list/inspect discover pending proposals (not filesystem search).${supportsCompletion ? " complete = durably finish this review after all proposal work." : ""} Nothing writes a live skill directly; lifecycle actions are unavailable.`
              : collectionOnly
                ? SKILL_COLLECTION_ACTION_DESCRIPTION
                : "create = new skill; read = existing live skill when complete content fits; prepare_patch = authorize one exact non-empty span and return bounded context, with only one prepared span active per skill; patch = targeted find-and-replace after read or prepare_patch; update = full-body rewrite after read; history = show up to 20 recent collection review outcomes and drop reasons; restore_collection = restore the collection backup retained by the last cleanup; revise = existing pending proposal; list/inspect discover pending proposals (not filesystem search); evaluate runs plugin evaluators for the exact draft; apply/reject/quarantine are explicit lifecycle actions.",
        },
      ),
