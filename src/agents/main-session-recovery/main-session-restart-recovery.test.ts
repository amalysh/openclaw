...
const mockRecoveryRuntime = {
  ...
  dispatchAgent: async <T>(
    ...
    if (status === undefined) {
      ...
      return { ...(result as object), status: "ok" } as T;
    }
    return result;
  },
...
async function expectRecovery(
  expected: { started: number; recovered: number; failed: number; skipped: number },
...
describe("main-session-restart-recovery", () => {
  ...
  it("marks active sessions in a configured custom session store", async () => {
    ...
    expect(recovery).toEqual({ started: 1, recovered: 0, failed: 0, skipped: 0 });
  });
  ...
  it("dispatches a bare fixed-store recovery under its persisted owner", async () => {
    ...
    ).resolves.toEqual({ started: 1, recovered: 0, failed: 0, skipped: 0 });
  ...
  it("resumes marked sessions with a tool-result transcript tail", async () => {
    ...
    await expectRecovery({ started: 1, recovered: 0, failed: 0, skipped: 0 });
  ...
  it.each([
    ...
    await expectRecovery({ started: 0, recovered: 0, failed: 0, skipped: 1 });
  ...
  it("retries when a human recovery run appears during announce reconciliation", async () => {
    ...
    try {
      await expectRecovery({ started: 0, recovered: 0, failed: 1, skipped: 0 });
    } finally {
  ...
  it("delivers resumed marked sessions through reply payload hooks", async () => {
    ...
    try {
      await expectRecovery({ started: 0, recovered: 1, failed: 0, skipped: 0 });
      const resumeParams = gatewayParams() as Record<string, unknown>;
  ...
  it.each([
    ...
    async (_label, cfg) => {
      ...
      await expectRecovery({ started: 1, recovered: 0, failed: 0, skipped: 0 }, cfg);
  ...
  it("keeps ambiguous pre-admission recovery identity unbound", async () => {
    ...
    await expectRecovery(
      { started: 0, recovered: 0, failed: 1, skipped: 0 },
    ...
    await expectRecovery(
      { started: 1, recovered: 0, failed: 0, skipped: 0 },
      executionIdentityEnabledConfig,
    );
    const runIds = vi
      .mocked(callGateway)
  ...
  it("preserves recovery identity when a prior-lifecycle delivery run rotates", async () => {
    ...
    expect(recoveryResult).toEqual({ started: 1, recovered: 0, failed: 0, skipped: 0 });
  ...
  it("retries reservation cleanup after a transient session-store failure", async () => {
    ...
    try {
      ...
      ).resolves.toEqual({ started: 0, recovered: 0, failed: 1, skipped: 0 });
    } finally {
  ...
  it("settles an admitted recovery that completed before its ambiguous response", async () => {
    ...
    await expect(recoverRestartAbortedMainSessions({ cfg: {}, stateDir: tmpDir })).resolves.toEqual(
      { started: 0, recovered: 1, failed: 0, skipped: 0 },
    );
    expect(loadSessionEntry({ sessionKey: "agent:main:main", storePath })).toMatchObject({
      abortedLastRun: false,
      status: "done",
  ...
  it("does not deliver restart recovery when session send policy denies sends", async () => {
    ...
    expect(result).toEqual({ started: 1, recovered: 0, failed: 0, skipped: 0 });
  ...
  it.each(["pending", "failed", "completed"] as const)(
    ...
    async (ownerStatus) => {
      ...
      try {
        ...
        await expectRecovery(
          ownerStatus === "pending"
            ? { started: 0, recovered: 0, failed: 0, skipped: 1 }
            : { started: 0, recovered: 1, failed: 0, skipped: 0 },
        );
  ...
  it("does not scan ordinary running sessions without the restart-aborted marker", async () => {
    ...
    await expectRecovery({ started: 0, recovered: 0, failed: 0, skipped: 0 });
    expect(callGateway).not.toHaveBeenCalled();
  });
  ...
  it("skips restart-aborted sessions that a current process owns", async () => {
    ...
    expect(result).toEqual({ started: 1, recovered: 0, failed: 0, skipped: 2 });
    expect(callGateway).toHaveBeenCalledOnce();
    const store = readStore(path.join(sessionsDir, "sessions.json"));
  ...
  it("marks startup-orphaned running main sessions before recovery", async () => {
    ...
    expect(recovered).toEqual({ started: 2, recovered: 0, failed: 0, skipped: 0 });
  ...
  it("does not report an execution-started recovery before its late terminal failure", async () => {
    const { sessionsDir, storePath } = await makeMainSessionFixture({
      restartRecoveryDeliveryRunId: "recovery-main",
      restartRecoveryDeliverySourceRunId: "source-main",
    });
    ...
    const terminal = createDeferred<{ runId: string; status: "error" }>();
        ...
        await withEnvAsync({ OPENCLAW_STATE_DIR: tmpDir }, async () => {
          await persistGatewaySessionLifecycleEvent({
            ...
            event: {
              ...
              runId: "recovery-main",
              lifecycleGeneration: getAgentEventLifecycleGeneration(),
              data: { phase: "error", error: "recovery ended without a terminal reply" },
            },
          });
        });
        return result;
    ...
    terminal.resolve({ runId: "recovery-main", status: "error" });
    await waitForFast(() => {
      expect(loadSessionEntry({ sessionKey: "agent:main:main", storePath })).toMatchObject({
        status: "failed",
        abortedLastRun: false,
  ...
  it("restores an accepted recovery that fails before execution starts", async () => {
    ...
    try {
      ...
      expect(result).toEqual({ started: 0, recovered: 0, failed: 1, skipped: 0 });
  ...
  it("holds lifecycle replacement behind the targeted recovery dispatch", async () => {
    ...
    try {
      ...
      releaseDispatch.resolve();
      await expect(recovery).resolves.toEqual({ started: 1, recovered: 0, failed: 0, skipped: 0 });
      await mutation;
  ...
  it("does not retry a replacement durable claim", async () => {
    ...
    expect(result).toEqual({ started: 0, recovered: 0, failed: 0, skipped: 0 });
    expect(callGateway).not.toHaveBeenCalled();
    expect(loadSessionEntry({ sessionKey: "agent:main:main", storePath })).toMatchObject({
      abortedLastRun: true,
      restartRecoveryDeliveryRunId: "replacement-recovery",
  ...
  it.each([
    {
      replaySafe: true,
      expected: { started: 1, recovered: 0, failed: 0, skipped: 0 },
      gatewayCalls: 1,
    },
    {
      replaySafe: false,
